...
But there's a problem with this definition. Suppose we are comparing two possibilities: having 100 words to eliminate, or having 10,000 words to eliminate. Clearly it is much worse to have 10,000 words, but how many times worse is it? If we used the above metric, it would be treated as 100 times as bad. But that seems far too pessimistic! What we ultimately care about is the number of turns it will take us to reach the answer, and we're not going to need 100 times as many turns (the Wordle game starts with over 10,000 possible solutions and it's not hard to whittle them down in 6 turns). We need to find an alternative way of measuring the size of the remaining set of words which doesn't penalise large numbers to quite the same extent.
