...
To sum up, for each letter, in each of the 5 positions, we can compute the Shannon entropy, which is the amount of information that using that letter in that position can be expected to give us, on average. We can calculate this from the probabilities of each of the 3 possible outcomes (green/orange/grey) using the formula above. The best letters in each position will be the ones which partition the set of words into 3 equal sized groups, consistent with each respective outcome. The worst letters are those which are almost certain to give a particular outcome (e.g. 'x' will almost certainly be grey).

We can perform this computation relatively quickly. We need to make one pass through the word list to sum up the number of occurrences of green/orange/grey for each letter in each position. That requires ~10,000 wordle goes to be simulated. But that's then enough to give us all the probabilities we need. After that we need one more pass through the word list to sum up the value of each word using the value of its letters. But that doesn't even need us to simulate any Wordle goes. It's just a big sum.
